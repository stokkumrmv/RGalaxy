...
#        roxygenize(dirToRoxygenize, roclets=("rd"))
    ...
    rd <- readLines(getManPage(manpage, package))
    title <- getTitle(rd)
    ...
    command.text <- gsub(funcName, paste0("'$__tool_directory__/", funcName), commandText)
    command.new <- paste0(c(command.pre, "\n    Rscript ", commandText))
    # command.new <- sub("2>&1([^2>&1]*)$", "", command.new)
    command.new <- xmlCDataNode(command.new)
    commandNode <- newXMLNode("command", newXMLTextNode(command.new),
                              parent=xml,
                              attrs = list(interpreter = "RScript --vanilla",
